/*L02-CANDELA*/
/* ./simple -i Candela/Candela_m1.10_%06d.pgm -t 500 */
#include "../utils.h"
...
int main(int argc, char** argv)
{
    ...
    cv::Mat foreground0, foreground1, foreground2;
    int thresh = 20;
	float alpha = 0.7;
    ...
    while (!exit_loop) {
        ...
        std::cout << "The image has " << image.channels() << " channels, the size is " << image.rows << "x" << image.cols << " pixels "
                  << " the type is " << matTypeCVstr(image) << " the pixel size is " << image.elemSize() << " and each channel is " << image.elemSize1() << (image.elemSize1() > 1 ? " bytes" : " byte") << std::endl;
        ...
        simpleBgSubtraction(image, precBackground, thresh, foreground0);
        cv::namedWindow("simpleBgSubtraction", cv::WINDOW_NORMAL);
        cv::imshow("simpleBgSubtraction", foreground0); 

        runningAverageBgSubtraction(image, frames, thresh, 5, foreground1);
        cv::namedWindow("AVGSubtraction", cv::WINDOW_NORMAL);
        cv::imshow("AVGSubtraction", foreground1);

        expRunningAverageBgSubtraction(image, 10, alpha, foreground2);
        cv::namedWindow("expAVGSubtraction", cv::WINDOW_NORMAL);
        cv::imshow("expAVGSubtraction", foreground2);
