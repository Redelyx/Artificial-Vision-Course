...
int main(int argc, char **argv)
{
  ...
  while (!exit_loop)
  {
    ...
    {
      std::cerr << "ERROR: t_low should be lower than t_high" << std::endl;
      exit(1);
    }
...
/*## Realizzare una funzione che genera un kernel Gaussiano per
smoothing verticale
- Krnl: singolo canale float32
- Sigma: deviazione standard gaussiana
- R: raggio del kernel, quindi il kernel avrà dimensione (2*r+1) x 1
...
/*Applicare quanto visto precedentemente e le regole di separabilità dei
filtri gaussiani per realizzare una funzione di smoothing gaussiano
bidimensionale*/
#define SEPARABLE
...
/*Basandosi su quanto già sviluppato, scrivere una funzione che
calcoli magnitudo e orientazione di un Sobel 3x3
- src uint8
- magn float32
- ori float32*/
...
Il risultato sarà un singolo valore float ottenuto interpolando i 4
vicini di (r,c)*/
template <typename T>
float bilinear(const cv::Mat &src, float r, float c)
{
...
/*Dati i risultati di Sobel applicati al risultato del filtro gaussiano calcolare i massimi del
gradiente in direzione perpendicolare al gradiente stesso
- magn: singolo canale float32
- ori: singolo canale float32
